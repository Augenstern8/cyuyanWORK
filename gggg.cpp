...
void zhuce()
{
	FILE *nchuhu=fopen("D:/daimaC/atm6.txt","r");
	FILE *fchuhu=fopen("D:/daimaC/atm6.txt","a");
	int i;
	if((nchuhu=fopen("D:/daimaC/atm6.txt","r"))==NULL) printf("文件无，系统创建\n"); 
	do
	{
		system("cls");
    	printf("\n\n\t\t请输入新储户的信息：");
...
void signUp()
{
	...
	FILE *fchuhu=fopen("D:/daimaC/atm6.txt","a");
	int i;
	if((nchuhu=fopen("D:/daimaC/atm6.txt","r"))==NULL) printf("No files, system created\n"); 
	do
	{
		system("cls");
...
int yanzheng()
{
	FILE *fchuhu=fopen("D:/daimaC/atm6.txt","r");
	int count=0;
	char id[9];
	for(i=0;i<=100;i++)
	{
	...
	for(i=0;i<=100;i++)
	{
		...
		for(i=0;i<=99;i++)
		{
			...
			{ 
				...
				fclose(fchuhu);
				fclose(achuhu);
				system("D:/daimaC/del atm.txt");
				rename("back.txt","D:/daimaC/atm6.txt");
				break;
	        }
